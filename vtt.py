...
from utils.text_cleanup import ArabicNormalizer
...
def vtt_to_whisper_result(source: Union[str, bytes]) -> WhisperResult:
    ...
    time_pattern = re.compile(r"(?P<start>\d{1,2}:\d{2}:\d{2}\.\d{2,3})\s*-->\s*(?P<end>\d{1,2}:\d{2}:\d{2}\.\d{2,3})")
    ...
    while i < n:
        ...
        if 'call' in source and start_sec == 0.0:
            start_sec = 0.5  # Adjust start time for call data.
        end_sec = parse_timestamp(end_ts)
        # end_sec -= 0.15  # Adjust end time to avoid overlap with next segment.

        # Gather all following lines until a blank line (which separates cues).
        text_lines = []
        i += 1
        while i < n and lines[i].strip():
            text_lines.append(lines[i] + '. ' if lines[i].strip()[-1] != '.' else lines[i])
            i += 1
        text = " " + " ".join(text_lines).strip()
        text_normalizer = ArabicNormalizer(text)
        text = text_normalizer.normalize_all().text
