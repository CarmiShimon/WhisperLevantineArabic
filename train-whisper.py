...
import evaluate
import torch
from datasets import DatasetDict, interleave_datasets, load_dataset, load_from_disk, concatenate_datasets
...
def parse_arguments():
    ...
    parser.add_argument(
        "--ds_processor_proc_num", type=int, default=32, help="Number of parallel processors for datasets preparation"
    )
    parser.add_argument("--model_name", default="openai/whisper-large-v3", help="Name of the model to train")
    parser.add_argument("--output_model_name", required=False, help="Name of the fine-tuned model to generate")
    ...
    parser.add_argument("--save_steps", type=int, default=2000, help="Number of steps between each model save/upload.")
    parser.add_argument(
...
def main():
    ...
    args.use_preprocessed = ['/shares/rndsounds/wake_up_word/genAI/ac/stt/verbit_dataset/large_v3_turbo']
    # args.train_datasets = ['/shares/rndsounds/wake_up_word/genAI/ac/stt/verbit_dataset/medium']
    # args.eval_datasets = ['/shares/rndsounds/wake_up_word/genAI/ac/stt/verbit_dataset/medium']
    # args.save_processed = '/shares/rndsounds/wake_up_word/genAI/ac/stt/verbit_large_v3/processed_verbit'
    # args.eval_datasets = '/shares/rndsounds/wake_up_word/genAI/ac/stt/Verbit/eval'  # for testing purposes, use a single dir
    # args.model_name = 'openai/whisper-large-v3-turbo'
    args.model_name = 'mboushaba/whisper-large-v3-turbo-arabic'
    args.output_model_name = '/shares/rndsounds/wake_up_word/genAI/ac/stt/Verbit/large_v3_turbo_2'  # for testing purposes, use a single dir
    args.mixed_precision = "fp16"
    # args.resume_from_checkpoint = True
    # args.resume_from_checkpoint_path = 'mboushaba/whisper-large-v3-turbo-arabic'  # /shares/rndsounds/wake_up_word/genAI/ac/stt/Verbit/large_v3/checkpoint-14000'
    # args.use_qlora = True
    args.lr_scheduler_type = 'linear'
    args.learning_rate = 3e-6
    args.ds_processor_proc_num = 32
    args.per_device_train_batch_size = 8
    args.per_device_eval_batch_size = 32
    args.gradient_accumulation_steps = 4
    args.max_eval_set_size = 200
    args.eval_steps = 1000
    args.predict_wer = True
    args.warmup_steps = 500
    args.save_steps = 2000
    args.run_name = 'Verbit_medium'
    args.save_only_model = True
    args.skip_push_to_hub = True
    args.include_timestamps_prob = 1.0
    args.include_prev_text_prob = 1.0
    ...
    if args.use_preprocessed:
        ...
        for preprocessed in args.use_preprocessed:
            try:
                # Load all train and eval parts
                train_parts = []
                eval_parts = []
                for i in range(35):  # adjust based on how many parts you have
                    train_parts.append(load_from_disk(f"{preprocessed}/train_part_{i}"))
                for i in range(1):  # adjust based on how many eval parts you have
                    eval_parts.append(load_from_disk(f"{preprocessed}/eval_part_{i}"))

                train_set = concatenate_datasets(train_parts)
                eval_set = concatenate_datasets(eval_parts)

                preprocessed_dataset_dicts.append({"train": train_set, "eval": eval_set})
        ...
        if len(preprocessed_dataset_dicts) == 1:
            dataset_dict = preprocessed_dataset_dicts[0]
    ...
    model.generate = partial(model.generate, language="arabic", task="transcribe", use_cache=True)
